...
def convert_sentence_to_features(sentence, tokenizer, max_seq_len):
    ...
    return np.array(input_ids).reshape(1, max_seq_length), np.array(input_mask).reshape(1, max_seq_length), np.array(
        segment_ids).reshape(1, max_seq_length)
...
def train_model(model, path_to_features, log_name, model_name, batch_size=32, step_per_epoch=10, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        f = open(log_name, "a")
        print(
            "Epoch {:03d}: Loss start : {:.3f},Loss end :  {:0.3f}, Accuracy_for_start: {:.3%} ,  Accuracy_for_end: {:.3%}".format(
                epoch, epoch_loss_avg1.result(), epoch_loss_avg2.result(), epoch_accuracy_start.result(),
                epoch_accuracy_end.result()))
...
def build_model(max_seq_length=512):
    ...
    # import modeling
    # import json
    # config_path = "local_model/uncased_L-2_H-128_A-2/bert_config.json"
    # f = open(config_path, "r", encoding="utf8")
    # config_file = json.load(f)
    #
    # Bert_config = modeling.BertConfig(**config_file)
    # bert_model = modeling.BertModel(Bert_config,is_training=True,input_ids=context_input_word_ids)
    #
    #

    bert_module = hub.Module("https://tfhub.dev/google/small_bert/bert_uncased_L-2_H-128_A-2/1",
                             trainable=True,
                             tags=None)
    ...
    output = bert_module(bert_inputs1,signature="tokens",as_dict=True)


    question_pooled_output, question_sequence_output =output["pooled_output"],output["sequence_output"]

    bert_inputs2 = dict(
        input_ids = context_input_word_ids,
        input_mask = context_input_mask,
        segment_ids = context_segment_ids)
    output = bert_module(bert_inputs2,signature="tokens",as_dict=True)
    context_sequence_output = output["sequence_output"]
    ...
    model = keras.Model(
        inputs=[question_input_word_ids, question_input_mask, question_segment_ids, context_input_word_ids,
                context_input_mask, context_segment_ids], outputs=[soft_max_salida_start, soft_max_salida_end],
        name="Luis_net")
...
N = 5000  # len(x)
entrada = {"questions_id": np.squeeze(x[:N, 3].astype(np.int32)),
...
#
model_callback = tf.keras.callbacks.ModelCheckpoint("local_model/model_e{epoch}-val_loss_{val_loss:.4f}.hdf5",
                                                    save_best_only=True)
...
model.fit(entrada, salida, batch_size=BATCH_SIZE, validation_split=0.1, epochs=10,
          callbacks=[model_callback, early_callback_start], verbose=1)
...
entrada = {"questions_id": np.squeeze(X_test[:, 3].astype(np.int32)),
           "question_input_mask": np.squeeze(X_test[:, 4].astype(np.int32)),
           "question_segment_id": np.squeeze(X_test[:, 5].astype(np.int32)),
           "context_id": np.squeeze(X_test[:, 0].astype(np.int32)),
           "context_input_mask": np.squeeze(X_test[:, 1].astype(np.int32)),
           "context_segment_id": np.squeeze(X_test[:, 2].astype(np.int32))}
