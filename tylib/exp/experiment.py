    ...
    def _load_embeddings(self):
        ...
        if(self.args.pretrained==1):
            if(self.args.use_openlm==1):
                self.mdl.load_weights(self.sess, self.openlm_path, 12)
            else:
                embedding = np.load('./datasets/{}/emb_{}_{}.npy'.format(
                    self.args.dataset, self.args.emb_type,
                    self.args.emb_size))
                # print(embedding)
                num_words = np.shape(embedding)[0]
                if(num_words<len(self.word_index)):
                    print("Supplementing Embedding Matrix.")
                    num_sup = len(self.word_index) - num_words
                    mat = np.zeros((num_sup, embedding.shape[1]))
                    embedding = np.vstack((embedding, mat))
                    print(embedding.shape)
                #print(embedding[5])
                #print(embedding[10])
                _sum = np.sum(embedding, axis=1)
                # print([x for x in _sum if x>0])
